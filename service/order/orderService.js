...
const orderService = {
    ...
    // Append new line items to an existing order. Reuses createOrder's
    // per-line allocation + pricing logic. After insert it recalculates
    // totals + promised_delivery_date and re-derives the parent order's
    // status (e.g. a previously-PACKED order regresses to PRODUCTION if
    // a new line needs production).
    //
    // GST note: this currently allows adding items at any non-terminal
    // status, including INVOICE/SHIPPED. That's fine while billing/tax
    // documents are not yet generated by this system. Once a real GST
    // invoice flow exists, restrict to statuses before INVOICE (same
    // rule as assertCancellable).
    addItemsToOrder: asyncHandler(async (orderNumber, items = []) => {
        const { employeeId, employeeRole } = getAuthenticatedEmployeeContext();
        ...
        if (TERMINAL.includes(order.status)) {
            throw new BadRequestException(
                `Cannot add items to order ${orderNumber} (status: ${order.status}).`
            );
        }

        // Auth: creator OR admin-privileged role.
        const isCreator = order.created_by === employeeId;
        const isPrivileged = ADMIN_PRIVILEGED_ROLES.includes(employeeRole);
        if (!isCreator && !isPrivileged) {
            throw new ForbiddenException(
                `Only the order creator or an admin/manager can add items to this order.`
            );
        }
        ...
        if (!dealer) {
            throw new BadRequestException(
                `Order ${orderNumber}'s dealer (${order.dealer_id}) not found.`
            );
        }

        const productIds = items.map((d) => d.product_id);
        const { productMap, productStockMap } = await productService.getProductsByIds(productIds);
        ...
        for (const detail of items) {
            ...
            if (isBattery) {
                ...
                const result = await productService.checkAndReserveStock(
                    product, stockDoc, qtyOrdered,
                    employeeId, employeeRole, orderNumber
                );
                productionRequired = result.productionRequired;
                packedUsed = result.packedUsed;
                unpackedUsed = result.unpackedUsed;
            ...
            if (isDiscountAllowed && detail.discount_price && Number(detail.discount_price) > 0) {
                unitDiscount = Number(detail.discount_price);
                discountNotes.push(`Manual Discount Applied: ${unitDiscount}`);
            } else if (isDiscountAllowed && detail.dealer_discount_id) {
                const dealerDiscount = await DealerDiscount.findOne({
                    dealer_discount_id: sanitizeInput(detail.dealer_discount_id),
                    dealer_id: dealer.employee_id,
                    brand_name: product.brand,
                ...
                if (dealerDiscount) {
                    const eligibleProducts = Array.isArray(dealerDiscount.product_ids)
                        ? dealerDiscount.product_ids : [];
                    if (eligibleProducts.includes(product.product_id)) {
                        if (dealerDiscount.is_percentage) {
            ...
            const notes = [
                ...
                unpackedUsed > 0 && `Unpacked Used: ${unpackedUsed}`,
                isBattery && `Battery Product: Direct PACKED allocation`,
                `Added to existing order by ${employeeRole}`,
                ...discountNotes,
            ].filter(Boolean).join(" | ");
        ...
        await OrderDetails.insertMany(newDetailsPayload);

        // Recompute order-level fields from the full (existing + new) detail set.
        const allDetails = await OrderDetails.find({ order_number: orderNumber });

        order.order_total_price = round(
            allDetails
                .filter((d) => !d.is_free)
                .reduce((sum, d) => sum + (Number(d.total_price) || 0), 0)
        );
        order.order_total_discount = round(
            allDetails
                .filter((d) => !d.is_free)
                .reduce((sum, d) => sum + (Number(d.total_dealer_discount) || 0), 0)
        );

        // Latest delivery_date across all details.
        const latestDate = allDetails
            .map((d) => new Date(d.delivery_date))
            .filter((d) => !Number.isNaN(d.getTime()))
            .reduce((latest, current) => (current > latest ? current : latest), new Date(0));
        if (latestDate.getTime() > 0) order.promised_delivery_date = latestDate;

        // If new lines need production but order had already moved past it,
        // regress to PRODUCTION so production team sees the work.
        if (hasPendingProduction && order.status !== ORDER_STATUSES.PENDING) {
            order.status = ORDER_STATUSES.PRODUCTION;
        } else if (order.status === ORDER_STATUSES.PENDING && employeeRole !== ROLES.SALESMAN) {
            // Admin/manager adding to a PENDING order auto-confirms the workflow.
            order.status = ORDER_STATUSES.PRODUCTION;
        }
